...
// Check if 'id' exists in the URL query string
if(isset($_GET['id'])) {
    //$car_id = $_GET['id'];
    $car_id = isset($_GET['id']) ? $_GET['id'] : null;

    // Now you can use $car_id safely
    echo "Car ID: " . $car_id;
} else {
    // Handle the case where 'id' is not passed in the URL
      ...
      <div class="form-area">
        ...
        if(mysqli_num_rows($result1)){
            while($row1 = mysqli_fetch_assoc($result1)){
                $car_name = $row1["CAR_NAME"] ?? "Not Available";
                $car_nameplate = $row1["CAR_NAMEPLATE"] ?? "Not Available";
                $ac_price = $row1["AC_PRICE"] ?? 0;
                $non_ac_price = $row1["NON_AC_PRICE"] ?? 0;
                $ac_price_per_day = $row1["AC_PRICE_PER_DAY"] ?? 0;
                $non_ac_price_per_day = $row1["NON_AC_PRICE_PER_DAY"] ?? 0;
